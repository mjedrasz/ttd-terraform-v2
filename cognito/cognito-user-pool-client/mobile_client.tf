...
resource "aws_ssm_parameter" "user_sign_in_callback_url" {
  ...
  name      = "/${var.aws_env}/cognito/user-pool-client/user/sign-in-url"
  type      = "String"
...
resource "aws_ssm_parameter" "user_sign_out_callback_url" {
  count     = var.route53 ? 1 : 0
  name      = "/${var.aws_env}/cognito/user-pool-client/user/sign-out-url"
  type      = "String"
...
resource "aws_ssm_parameter" "user_pool_client_id" {
  name  = "/${var.aws_env}/cognito/user-pool-client/user/id"
  type  = "String"
